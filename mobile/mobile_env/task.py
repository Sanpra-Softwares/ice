...
def create_task(**kwargs):
    try:
        ...
        # Check if task 'name' is provided, fetch the existing task if it exists
        if data.get("name"):
            # Attempt to fetch the existing task by name (ID)
            task_doc = frappe.get_doc("Task", data.get("name"))
            message = "Task has been updated successfully"
        else:
            # Create a new Task document if 'name' is not provided
        ...
        # Insert or save the task document
        if data.get("name"):
        ...
        return gen_response(200, message)
    
    except frappe.DoesNotExistError:
        return gen_response(404, "Task not found with the provided name.")
    except Exception as e:
